...
=== Solution
Our approach unfolds like a story across four chapters, each building upon the last, with an optional Chapter 0 in front of them for readers who want the fundamentals first.
...
This chapter is optional and has no Fitnet code. It covers the theory the other chapters assume you already know.

What we cover:

- What architecture is, and where it ends and design begins
- Coupling and cohesion, and what a boundary costs once it exists
- Requirements, quality attributes and constraints
- Turning "it should be fast" into a number you can test
- Recording decisions in an architecture decision log, and drawing them with C4

If you have never had to defend an architecture decision to anyone, read this first. If the terms above are familiar, start at Chapter 1 and come back when one of them catches you off guard.

More information:

- link:/Chapter-0-software-architecture-fundamentals/README.adoc[Chapter's readme]
...
This repository is organized with crystal-clear simplicity: beyond this main README and its supporting assets, you will find four key chapters that tell our evolutionary story, preceded by an optional Chapter 0 with the fundamentals:

In each folder you will find the same root solution, but expanded:

- Chapter 0: The theory behind the decisions that follow - no code, optional
...
IMPORTANT: While you are free to explore these chapters in any order, we strongly recommend starting with Chapter 1 (or Chapter 0, if the fundamentals are new to you) and progressing sequentially. Like any good story, the earlier chapters lay crucial groundwork that makes later developments more meaningful. Jumping ahead might leave you missing important context and concepts.
