...
This chapter is optional, and it is the one to read first if you have never had to defend an architecture decision to anyone. It covers no Fitnet code at all.

Instead it builds the vocabulary the rest of the story assumes you already have: what separates an architectural decision from a design one, why every choice is a trade-off, how to turn "it should be fast" into a number you can test, and how to write the reasoning down so the next person does not have to guess at it.

If those already sound familiar, start at Chapter 1 and come back only when a term catches you off guard.
