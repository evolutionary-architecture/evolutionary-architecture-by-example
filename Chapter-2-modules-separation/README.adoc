...
1. Our application is used by 5000 people (the upper limit of our first assumptions from the MVP). It still makes no sense to extract parts of the application into microservices, so we keep our modular monolith and scale it as a single deployment unit.
