...
[UsedImplicitly]
internal sealed class PrepareContractCommandHandler
    (IContractsRepository contractsRepository) : IRequestHandler<PrepareContractCommand, Guid>
{
    public async Task<Guid> Handle(PrepareContractCommand command, CancellationToken cancellationToken)
    {
        ...
        var contract = Contract.Prepare(command.CustomerId, command.CustomerAge, command.CustomerHeight,
            command.PreparedAt, previousContract?.IsSigned);
