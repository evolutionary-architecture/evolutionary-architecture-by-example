...
It is written for 3 kinds of readers: engineers who aspire to become architects, tech leads who already make these calls without the title, and architects who want to revisit the fundamentals.

If you aspire to the role, you already know your language, your framework, and the awkward corners of your codebase. What is missing is not coding skill. It is a way of reasoning about questions that have no lookup answer (1 service or 3, shared database or a schema per module, is this pattern worth the complexity it adds) and the technical breadth to judge options built on technology you have never run in production. Nobody teaches either one on purpose.

If you are a tech lead, you already make several of these decisions every month, usually under a deadline and usually without anyone in the room calling them architecture. What this chapter adds is the vocabulary to defend them and the instruments to tell the expensive ones apart from the cheap ones before you commit.

If you already hold the title, little of this will be new, and that is the point. It is the vocabulary and the reasoning the rest of this repository assumes, collected in one place, so you can check your own defaults against it before the chapters start making decisions.

So this chapter's purpose is narrow: prepare you to understand the decisions the rest of this repository makes, not walk you through making them yet. Chapter 1 is where that thinking gets applied to a real system for the first time, and every chapter after it assumes you already reason this way.
...
In code it looks the same. When `Fitnet.Passes` calls a method on `Fitnet.Contracts`, it has to know 3 things: that the module exists, what its method looks like, and that it is running right now. Change any of the 3 and both sides change together. When `Fitnet.Passes` only reacts to an event instead, it has to know 1 thing, the shape of the message, and the other module can be rewritten, restarted or moved without asking permission.
...
Four laws keep coming back in every decision we make in the chapters ahead. We name them here so that later, when one of them is doing the work, you recognize it instead of reading it as an opinion. The first 2 are stated as laws of software architecture by Richards and Ford. The other 2 come from elsewhere, Conway's 1967 paper and the evolutionary architecture work this repository takes its name from, and we group them here because in practice they carry the same weight.
...
== One Decision, From Start To Finish

Every instrument above works on its own. Watching them work together on a single decision is what turns them into a mindset, so here is a decision Chapter 1 actually makes, run through all of them in order. Nothing in this section is hypothetical, and you can read the result of it in Chapter 1's code.

**The requirement.** When a customer signs a contract, a pass has to be created for them. Two modules are involved, `Fitnet.Contracts` and `Fitnet.Passes`, and the open question is how the first one tells the second.

**Is it even architecture?** Apply the cost-of-change test. Turning a direct method call into an event later means rewriting every integration point between the 2 modules, and the failure modes change with it. That is weeks of work, not an afternoon, so this is architecture and it deserves the full treatment below.

**The quality attributes, with metrics.** Chapter 1's assumptions give us 5000 users, about 625,000 requests a day, roughly 7 per second. From that we can write scenarios instead of opinions:

- Performance: signing a contract responds in under 300ms at the 95th percentile at 7 requests per second.
- Reliability: during the MVP we accept losing an in-flight event when the process restarts, at most a handful per month, repaired by hand. That is a deliberately weak target, and writing it down as a number is what makes it a decision instead of an accident.
- Maintainability: a new engineer can follow the whole signing process inside 1 folder.

**The trade-off.** An in-memory event bus costs nothing to run, adds no new component to operate, and delivers immediately. It also loses messages on restart and cannot replay them. A broker like RabbitMQ buys durability and replay, and charges for it in infrastructure, a new failure mode to learn, and a deployment step nobody on this team has done before. There is no option here without a bill attached, which is the first law showing up exactly where you would expect it.

**The cost.** In-memory: zero to run, a few days to build, and a rewrite later if we are wrong. A broker on day 1: a service to pay for every month, plus the hours nobody spends on features while learning it. At 5000 users, with a business that has told us it will cancel the project if the MVP does not prove itself, the second bill buys insurance for a system that may not exist in 6 months.

**The risk.** Score it on the 2 axes. Losing an in-flight event during the MVP is likely, and the impact is low, because the number of affected customers is small and support can fix a missing pass by hand. The same event loss at 10 times the traffic is equally likely and no longer low impact. So the risk is acceptable now and stops being acceptable at a threshold we can name in advance, which is far more useful than "we will revisit it later".

**The decision, and the consequences we accept.** We use the in-memory bus, and the ADR says so in writing: checkout stays simple and cheap, we lose messages on restart, and we owe ourselves a plan for the day that stops being tolerable.

**The fitness function.** The whole trade-off depends on `Fitnet.Passes` and `Fitnet.Contracts` staying independent, so a test asserts there is no compile-time reference between them. Without it, 6 months of "just this once" turns the cheap decision into an expensive one while everyone is looking somewhere else.

**When it gets revisited.** Exactly as planned. Chapter 2 keeps the in-memory queue but starts feeling the coupling, and Chapter 3 replaces it with RabbitMQ and MassTransit once the system is split into 2 deployment units and enough people are using it that lost messages are no longer a shrug. The decision did not fail. Its consequences arrived, on a trigger we had already written down.

That sequence, requirement to metric to trade-off to cost to risk to record to test to trigger, is the loop. Run it on a decision of your own and the abstract sections above turn into a habit.
...
Now it is time to put this mindset to work on a real system. In Chapter 1, we start Fitnet from the simplest architecture that could possibly work, and watch _The Project Paradox_ play out in practice.
