...
Where you sit on this spectrum can change month to month depending on what the team needs. An architect deep in a critical migration might code daily for weeks. An architect coordinating 4 teams through a quarter-end deadline might code far less, and lean harder on code review and pairing instead. What ruins credibility is not spending less time in the code - it is spending none, for long enough that you stop knowing what you are actually asking the team to build.

IMPORTANT: Staying hands-on is not free either - every hour writing code is an hour not spent riding the elevator between business and engineering. The Ivory Tower trap and the architect who never leaves the codebase are the same failure from opposite ends: both have stopped doing half the job.
