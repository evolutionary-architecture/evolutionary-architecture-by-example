...
internal static class RegisterEndpoint
{
    ...
    {
        app.MapPost(ApiPaths.Passes.Register, async (RegisterPassRequest request, PassesPersistence persistence) =>
        {
            var pass = Pass.Register(request.CustomerId, request.From, request.To);
            await persistence.Passes.AddAsync(pass);
            await persistence.SaveChangesAsync();

            return Results.Created($"/{ApiPaths.Passes.Register}/{pass.Id}", pass.Id);
        });
    }
}