...
== Run the Fitnet ModularMonolith Application

There are two options to run the Fitnet ModularMonolith application:

=== Option 1: Launch the Fitnet ModularMonolith application in an IDE

If you have JetBrains Rider installed, you can launch the Fitnet ModularMonolith application directly from there following these steps:

. Open the project in JetBrains Rider.
. Run the project.
. The Fitnet ModularMonolith application should start running.
...
=== Option 2: Build and run the Fitnet ModularMonolith application as a Docker container

. Build the Docker image:

[source,shell]
----
docker build -t fitnet_modularmonolith .
----
...
[source,shell]
----
docker run -p 8080:80 --name fitnet_modularmonolith_container fitnet_modularmonolith
----
...
To launch the Fitness.Contracts application in JetBrains Rider, follow these steps:

. Open the project in JetBrains Rider.
. Run the project.
. The Fitness.Contracts application should start running.
...
. Open JetBrains Rider and navigate to `File > Settings > NuGet > Sources`.
. Click the `+` button to add a new package source.
. In the `Add Package Source` window, provide Artifactory URL in the `https://nuget.pkg.github.com/evolutionary-architecture/index.json`, fill your Github Username and PAT.
. Click `OK` to confirm the new package source.
. Make sure your new package source is enabled and then click `OK` to close the `Settings` window.
...
Running integration tests for both the Fitnet ModularMonolith and Fitness.Contracts applications involves similar steps, as the testing setup for both projects.
