...
We did Event Storming Design Level to understand logic and business rules and their connections.
...
Annexes are a part of the binding contract aggregate. They has to be uniquely identified that why are modeled as entities. Two create same annexes can't be treated as the same annex because they have different identity.

Binding contract has signature property which has bussiness logic and is no requirment to be uniqly identified. That's why it is modeled as value object. Signature can be compared by its properties.
