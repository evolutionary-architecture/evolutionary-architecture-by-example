...
=== Annexes Tactical Design Analysys
TODO describe it rules and challanges show solution

Rules:
Annex Can Only Start During Binding Contract Period
Annex Can Only Be Attached To Active
Previous Annex has to be signed

"Previous Annex has to be signed" - this role is connected with reltion ship between new and previes annex.
We need way to protect this invariant. In concurrent environment. Breaking this rule could make that inconsitent.

That why binding contract enity has to become Aggregate Root 
that will guard annexes invariants.
