...
2. After the _Contract_ is signed, ___Binding Contract___ is created. It has the force of law and binds the customer with us.
3. After three months, at any time, the customer can terminate ___Binding Contract___ without any penalty.
4. It is possible to attach an _Annex_ to the existing ___Binding Contract___. This way, the customer can extend the contract for another year without preparing a new _Contract_ for him.
5. Annex can only be attached if ___Binding Contract___ is active - has not yet expired or was not terminated.
...
1. **Annex Can Only Start During Binding Contract Period**:
   - An annex can only be attached if it falls within the active period of the ____Binding Contract____. This ensures that all extensions and modifications are valid within the contract's timeframe.

2. **Annex Can Only Be Attached To Active Contracts**:
   - The ____Binding Contract____ must be active, meaning it hasn't expired or been terminated. This rule prevents any modifications to contracts that are no longer valid.
...
__Binding Contract__ has signature property which has bussines logic and is no requirment to be uniqly identified. Signature can be compared by its properties. We want to use ubiquites languge so we chosen <<value-object, value object>> as bulidng block to model this concept.
