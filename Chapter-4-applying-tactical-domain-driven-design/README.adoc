...
Domain-Driven Design (DDD) is a set of techniques and methods that help developers tackle complex systems full of business language that is not easy to understand for people not familiar with the business area or processes. DDD is particularly useful in handling business complexity. However, it is not recommended for trivial business cases, such as when the complexity lies only in the technology itself, or the application is just CRUD-based and can be designed as simple UI mockups.
...
TODO: Image Domain-Driven Whirlpool image

The second category is tactical, focusing on implementing solutions that we already understand in a way that is easy to maintain, explicit, and uses a ubiquitous language. Tactical techniques enforce business rules that match real business needs.

[NOTE]
====
We are using Tactical Domain-Driven Design from the start to understand and model the problem. Tactical patterns are used only in the _Contracts_ microservice due to its complexity.
====
