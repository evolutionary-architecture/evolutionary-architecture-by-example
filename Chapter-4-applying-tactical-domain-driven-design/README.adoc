...
[Note]
====
Domain Driven Design is collection of techniques that help us to build complex software systems. You don't have to use all of them to be successfully. Treat DDD as pharmacy.You should choose the ones that fit your needs and the complexity of the system you are building.
====
...
Above is the result of the Event Storming workshop. We did this section to model the business processes and rules in the _Contracts_.

Understanding these rules is crucial for ensuring our design maintains consistency and adheres to the business requirements.
...
Annexes are a part of the binding contract <<aggregate,aggregate>>. They has to be uniquely identified that why are modeled as entities. Two create same annexes can't be treated as the same annex because they have different identity.

Binding contract has signature property which has bussiness logic and is no requirment to be uniqly identified. That's why it is modeled as <<value-object, value object>>. Signature can be compared by its properties.
